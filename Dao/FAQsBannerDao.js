...
const createCareers = async (career) => {
    const [id] = await db('faq_banner').insert({
...
const getAllCareeres = async () => {
    return await db('faq_banner').select('*').orderBy('id', 'desc');
};

// Get career by ID
const getCareerByIds = async (id) => {
    return await db('faq_banner').where({ id }).first();
};

// Update career by ID
const updateCareerByIds = async (id, career) => {
    const result = await db('faq_banner')
        .where({ id })
        .update({
            sub_heading: career.sub_heading,
            image_carousel: career.image_carousel,
...
const deleteCareerByIds = async (id) => {
    const result = await db('faq_banner').where({ id }).del();
    return result;
};
