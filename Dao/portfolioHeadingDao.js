...
const createAboutBanner = async (banner) => {
    const [id] = await db('portfolio_heading').insert({
...
const getAllAboutBanners = async () => {
    return await db('portfolio_heading').select('*').orderBy('id', 'desc');
};

// Get about banner by ID
const getAboutBannerById = async (id) => {
    return await db('portfolio_heading').where({ id }).first();
};

// Update about banner by ID
const updateAboutBannerById = async (id, banner) => {
    const result = await db('portfolio_heading')
        .where({ id })
        .update({
            sub_heading: banner.sub_heading,
            status: banner.status,
...
const deleteAboutBannerById = async (id) => {
    const result = await db('portfolio_heading').where({ id }).del();
    return result;
};
