...
const createPortfolios = async (subHeading, image, textOnImage, status) => {
    ...
    return db('culture_portfolio').insert({
...
const getAllPortfolioes = async () => {
    return db('culture_portfolio').select('*').orderBy('created_at', 'desc');
};

// Get portfolio by ID
const getPortfolioByIds = async (id) => {
    return db('culture_portfolio').where({ id }).first();
};

// Update a portfolio item
const updatePortfolios = async (id, subHeading, image, textOnImage, status) => {
    return db('culture_portfolio')
        .where({ id })
        .update({
            sub_heading: subHeading,
            image: image,
...
const deletePortfolios = async (id) => {
    return db('culture_portfolio').where({ id }).del();
};
