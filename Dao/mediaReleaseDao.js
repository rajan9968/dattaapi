...
const createPressReleaseDao = async (pressRelease) => {
    const [id] = await db('media_coverages_detail').insert({
...
const getAllPressReleasesDao = async () => {
    const pressReleases = await db('media_coverages_detail').select('*').orderBy('id', 'desc');
    return pressReleases;
};

// 🔍 Get press release by ID
const getPressReleaseByIdDao = async (id) => {
    const pressRelease = await db('media_coverages_detail').where({ id }).first();
    return pressRelease;
};

// ✏️ Update press release
const updatePressReleaseByIdDao = async (id, updatedData) => {
    const result = await db('media_coverages_detail').where({ id }).update(updatedData);
    return result;
};

// ❌ Delete press release
const deletePressReleaseByIdDao = async (id) => {
    const result = await db('media_coverages_detail').where({ id }).del();
    return result;
};
